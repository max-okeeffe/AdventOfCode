...
let ends = List.map ~f:format @@ String.split_lines data

let overlap h z =
  match h, z with
  | [[a;b];[c;d]], [x;y] when a = c -> x = a && min b d <= y && y <= max b d
  | [[a;b];[c;d]], [x;y] when b = d -> y = b && min a c <= x && x <= max a c
  | [[a;b];[c;d]], [x;y] -> y - b = ((d-b) / (c-a)) * (x - a)  && min a c <= x && x <= max a c && min b d <= y && y <= max b d
  | _ -> false
...
let expand l =
  ...
  | [[a;b];[c;d]] when (d-b) / (c-a) = 1 -> List.init (abs (c-a) + 1) ~f:(fun i -> [i + min a c; i + min b d])
  | [[a;b];[c;d]] when (d-b) / (c-a) = -1 -> List.init (abs (c-a) + 1) ~f:(fun i -> [i + min a c; -i + max b d])
  | _ -> [[]]
