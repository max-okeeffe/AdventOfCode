...
let split_list l =
  let rec aux out curr l = match l with
    | [] -> out
    | "" :: t -> aux (out @ [curr]) [] t
...
let make_board board =
  let rec aux board curr = match board with
  | [] -> curr
  | h::t -> [| Array.map ( Array.of_list @@ List.filter (String.split h ~on:' ') ~f:(fun x -> not @@ String.equal x "")) ~f:Int.of_string |]
    |> Array.append curr
    |> aux t in
  aux board [||]
...
let rec get_winning_board boards = 
  ...
  let wins board = winning_row board || winning_row @@ Array.transpose_exn board in match boards with
  | [] -> [| [||] |]
  | h :: _ when wins h -> h
  | _ :: t -> get_winning_board t

let score board =
  let aux row = Array.fold (Array.filter row ~f:(fun i -> i <> -1)) ~init:0 ~f:(+) in
  aux @@ Array.map board ~f:aux

let rec game nums boards = match nums with
  | [] -> 0
  | h::t -> let play_h = play h boards in let get_winning = get_winning_board play_h in match get_winning with
    | [| [||] |] -> game t play_h
    | _ -> (score get_winning) * h
